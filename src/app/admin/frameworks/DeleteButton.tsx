...
/**
 * Delete button with confirmation for regulatory frameworks.
 * Shows "Delete?" → "Yes / No" on click before actually deleting.
 */
...
export function DeleteFrameworkButton({ id, name }: { id: string; name: string }) {
  ...
  return (
    <button onClick={() => setConfirming(true)} className="rounded-lg px-3 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50">
      Delete
    </button>
  );
}
