...
/**
 * VendorPrepClient — Interactive vendor meeting prep generator.
 *
 * Steps:
 *   1. Describe your meeting (conversational context textarea)
 *   2. Select an AI system, meeting type, attendee roles, concerns
 *   3. Add meeting title, agenda, key concerns
 *   4. Generate (loading state -> display briefing materials)
 *   5. Copy All / Print buttons
 */
...
const SECTION_CONFIG: Record<string, { emoji: string; color: string; bgColor: string; borderColor: string }> = {
  ...
  "intelligence": { emoji: "\ud83d\udcca", color: "text-indigo-700", bgColor: "bg-indigo-50", borderColor: "border-indigo-200" },
...
export function VendorPrepClient({ tier, systems }: Props) {
    ...
    try {
      const res = await fetch("/api/vendor-prep", {
        ...
        body: JSON.stringify({
          ...
          keyConcerns: keyConcerns || undefined,
        }),
      });
  ...
  return (
    <div>
      {/* Conversational Context — chat-style textarea at the top */}
      <div className="mb-6 rounded-xl border border-[#003399]/20 bg-white p-6 shadow-sm">
        <h3 className="text-sm font-semibold text-[#003399] uppercase tracking-wider mb-1 flex items-center gap-2">
        ...
        <textarea
          ...
          placeholder="Describe your upcoming vendor meeting — who's attending, what's the goal, what concerns you have. For example: 'We have a second demo with Microsoft about Copilot next Tuesday. Our CTO and DPO will attend. Main concern is data residency — we need EU-only processing. They're pushing a 3-year contract but we want flexibility...'"
          rows={4}
          className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:border-[#003399] focus:ring-1 focus:ring-[#003399] transition-colors resize-y"
        />
      </div>
      ...
      <div className="mb-6 grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <input
            ...
            placeholder="What's the meeting about? e.g. Copilot Enterprise Demo #2"
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <textarea
            value={keyConcerns}
            onChange={(e) => setKeyConcerns(e.target.value)}
            placeholder="What are your main concerns or priorities for this meeting?"
            rows={2}
            className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:border-[#003399] focus:ring-1 focus:ring-[#003399] transition-colors resize-y"
          />
        </div>
      ...
      <div className="mb-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="text-xs text-gray-500 mb-2">
          Paste the meeting agenda or describe what will be discussed. This helps generate talking points per agenda item.
        </p>
        <textarea
          value={meetingAgenda}
          onChange={(e) => setMeetingAgenda(e.target.value)}
          placeholder={"1. Product overview & demo\n2. Data residency and GDPR compliance\n3. Integration with existing tools\n4. Pricing and contract terms\n5. Next steps"}
          rows={4}
          className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:border-[#003399] focus:ring-1 focus:ring-[#003399] transition-colors resize-y font-mono text-xs"
        />
      </div>
      ...
      {loading && (
        <div className="mt-6 rounded-xl border border-[#003399]/20 bg-[#003399]/5 p-6 text-center">
          <div className="inline-flex items-center gap-3 text-[#003399]">
            ...
            <span className="font-medium">Analysing compliance data and preparing your meeting briefing...</span>
          </div>
