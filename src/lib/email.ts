...
const FROM_EMAIL = process.env.FROM_EMAIL || "VendorScope <noreply@vendorscope.eu>";
...
export async function sendMagicLink(email: string, token: string) {
  ...
  const subject = "Sign in to VendorScope";
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
      <div style="text-align: center; margin-bottom: 32px;">
        <h1 style="color: #003399; font-size: 24px; margin: 0;">VendorScope</h1>
      ...
      <p style="font-size: 11px; color: #aaa; text-align: center;">
        VendorScope &mdash; Independent AI compliance intelligence for Europe.<br/>
...
export async function sendDigest(email: string, subscriberId: string, items: DigestItem[]) {
  ...
  const subject = `VendorScope Digest — ${items.length} update${items.length !== 1 ? "s" : ""} on your topics`;
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
      ...
      <p style="font-size: 11px; color: #aaa; text-align: center;">
        ...
        VendorScope &mdash; AI Intelligence for European Decision-Makers
      </p>
    </div>
  `;
