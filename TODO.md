...
- [ ] **Register vendorscope.eu** (CEO decision 2026-05-03 after 6-iteration board brainstorm). Recommended registrar: Gandi.net (~€10/yr) or OVH. Optional defensive: vendorscope.ai (~€100/yr). Skip .com (parked, aftermarket only). Once registered, downstream rebrand work begins: site metadata, all 14 dictionaries, Vercel domain, LemonSqueezy config, email (corrections@), social handles. Brand: VendorScope. Tagline territory: research-firm tone, "we vet AI vendors against EU regulations". CRITICAL: this is a ~1-day refactor across the codebase once the domain is live — track as separate task.
...
- [ ] **P0 — Inline source links at point-of-use**: Every factual value on a system page (certifications, data residency, subprocessors, DPA availability, etc.) should be clickable — opens the exact vendor page/section in a new tab. Today the SourceChip only renders inside the separate "Evidence Sources" panel at the top; the accordions below render the same facts as plain text with no link. UX rule: if it's a claim, it's a link. Two tiers: (a) fact-level deep link when a published SystemClaim exists for that field → link to claim.source.url; (b) vendor trust-portal fallback when we only have a Tier-1 Source but no fact-level claim yet. Either way: `target="_blank" rel="noopener"`. Blocked on evidence backfill for (a). (added 2026-04-16 from CEO feedback — "make ISO 27001 clickable to the exact Salesforce page")
...
- [ ] **Risk assessment for running the site**: Identify operational, legal, reputational, financial risks of operating VendorScope. Include remediation plan, controls, metrics to measure risk. (from Telegram)
